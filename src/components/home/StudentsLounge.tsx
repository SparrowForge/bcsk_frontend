      ...
      <ul className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {tiles.map((tile) => (
          <li key={tile.label}>
            <Link
              href={tile.href}
              className="block rounded-lg bg-peach hover:bg-sunrise hover:text-white text-navy text-center text-[12px] font-bold px-3 py-2.5 transition-colors"
            >
              {tile.label}
            </Link>
          </li>
      ...
      {photos.length > 0 && (
        <>
          ...
          <ul className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {photos.map((p) => (
              <li key={p.id}>
                <Link href="/events/gallery" className="group block rounded-lg overflow-hidden border border-line">
