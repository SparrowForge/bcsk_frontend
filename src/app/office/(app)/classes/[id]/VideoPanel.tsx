    ...
    <section className="bg-white rounded-2xl border border-line p-6">
      ...
      <form action={action} className="bg-mist rounded-xl p-4 flex flex-wrap gap-3 items-center mb-5">
        ...
        <input
          ...
          className="flex-1 min-w-48 rounded-lg border border-line px-3.5 py-2.5 text-sm focus:border-green-mid focus:outline-none"
        />
        <input
          name="videoUrl"
          required
      ...
      <ul className="space-y-2">
        ...
        {videos.map((v) => (
          <li key={v.id} className="flex items-center gap-3 text-sm border border-line rounded-xl px-4 py-3">
            ...
            <a href={v.url} target="_blank" rel="noopener noreferrer" className="font-bold text-green hover:text-green-mid">
              {v.title}
            </a>
