  ...
  return (
    <form action={action} className="flex flex-wrap gap-3 items-start">
      ...
      <textarea
        ...
        className="flex-1 min-w-56 rounded-lg border border-line px-3.5 py-2.5 text-sm focus:border-green-mid focus:outline-none"
      />
      <button
        disabled={pending}
      ...
      {state?.ok && <p className="w-full text-xs font-semibold text-green">Reply sent — the student was notified.</p>}
    </form>
  );
}
