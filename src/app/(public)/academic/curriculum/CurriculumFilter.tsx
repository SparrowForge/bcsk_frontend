      ...
      <div className="flex flex-wrap gap-2 mb-8" role="tablist" aria-label="Filter by class">
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${
            selected === "ALL" ? "bg-green text-white" : "bg-mist text-green hover:bg-mist-deep"
          }`}
        >
          All classes
        </button>
        {classes.map((c) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${
              selected === c.key ? "bg-green text-white" : "bg-mist text-green hover:bg-mist-deep"
            }`}
          >
            {c.label}
          </button>
      ...
      <div className="space-y-6">
        {visible.map((c) => (
          <section key={c.key} id={c.key.toLowerCase()} className="bg-white border border-line rounded-2xl p-6">
            <h2 className="font-display text-xl font-semibold text-green">{c.label}</h2>
            <ul className="mt-4 grid sm:grid-cols-2 gap-2.5">
              {c.subjects.map((s) => (
                <li key={s.id} className="flex items-center gap-2.5 text-sm text-ink">
                  <span className="w-2 h-2 rounded-full bg-crimson shrink-0" aria-hidden />
