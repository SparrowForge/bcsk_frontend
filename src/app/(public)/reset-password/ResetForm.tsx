...
export function ResetForm({ token }: { token: string }) {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-3xl border border-line shadow-sm p-8">
      <h1 className="font-display text-2xl font-semibold text-green text-center">Set a new password</h1>
      {done ? (
        <div className="mt-6 text-center">
          <p className="text-sm text-ink-soft">Your password is updated. You can log in now.</p>
          <Link href="/classroom" className="inline-block mt-4 bg-green text-white font-bold rounded-lg px-6 py-2.5 text-sm">
            Go to login
          </Link>
        </div>
      ) : (
