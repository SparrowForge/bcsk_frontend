  ...
  return (
    <>
      <section className="mx-auto max-w-7xl px-4 pt-6">
        <div className="relative bg-green-mid rounded-3xl px-6 sm:px-12 py-14 overflow-hidden text-white">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {SKILLS.map((s) => (
            <div key={s.name} className="bg-white border border-line rounded-2xl p-6">
              ...
              <h3 className="mt-3 font-display text-lg font-semibold text-green">{s.name}</h3>
      ...
      <section className="mx-auto max-w-7xl px-4 mt-16 grid lg:grid-cols-2 gap-10">
              ...
              return (
                <details key={l.id} className="bg-white border border-line rounded-2xl group">
                  <summary className="cursor-pointer px-6 py-4 font-bold text-green flex items-center gap-3">
                    {l.name}
                    {session && (
                      <span className="text-[11px] font-bold bg-green-soft text-green rounded-full px-2.5 py-1">
                  ...
                  <div className="px-6 pb-5 text-sm text-ink-soft leading-relaxed">
                    ...
                    {session && enrolled && session.zoomLink && (
                      <a href={session.zoomLink} target="_blank" rel="noopener noreferrer" className="block mt-3 text-green font-bold hover:underline">
                        Join the live class →
                      </a>
                    )}
                  </div>
        ...
        <div>
          ...
          <div className="bg-mist rounded-3xl p-8">
            ...
            <Link
              href="/apply/special?course=ielts-for-kids"
              className="block mt-5 bg-green hover:bg-green-deep text-white text-center font-bold rounded-lg px-6 py-3 text-sm transition-colors"
            >
              Enroll now
            </Link>
          </div>
