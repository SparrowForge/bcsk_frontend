    ...
    <PageShell title={paid ? "Application complete!" : "Application received"} eyebrow="Application">
      <div className="max-w-xl">
        <div className="bg-white border border-line rounded-3xl p-8 text-center">
          <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center ${paid ? "bg-green/15 text-green" : "bg-amber/15 text-amber-ink"}`}>
            {paid ? (
          ...
          <h2 className="mt-4 font-display text-2xl font-semibold text-green">
          ...
          {payment && (
            <dl className="mt-6 bg-mist rounded-xl p-4 text-sm text-left space-y-1.5">
              ...
              <div className="flex justify-between"><dt className="font-bold text-ink-soft">Amount</dt><dd className="font-bold text-green">{krw(payment.amount)}</dd></div>
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            {!paid && !pending && (
              <Link href={`/apply/payment/${app.id}`} className="bg-crimson hover:bg-crimson-deep text-white font-bold rounded-lg px-6 py-2.5 text-sm">
                Complete payment
              </Link>
            )}
            {paid && (
              <Link href="/classroom" className="bg-crimson hover:bg-crimson-deep text-white font-bold rounded-lg px-6 py-2.5 text-sm">
                Open Classroom portal
              </Link>
            )}
            <Link href="/" className="bg-green hover:bg-green-deep text-white font-bold rounded-lg px-6 py-2.5 text-sm">
              Back to home
            </Link>
          </div>
        </div>
