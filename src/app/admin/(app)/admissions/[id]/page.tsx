...
export default async function AdmissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const app = await admissions.one(Number(id)).catch((e: unknown) => {
    // A missing application is a 404 page, not an error boundary.
    if (e instanceof ApiError && e.status === 404) notFound();
    throw e;
  });
  // `/admissions/:id` always includes the payment attempts; newest first is a page concern.
  const payments = [...(app.payments ?? [])].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
