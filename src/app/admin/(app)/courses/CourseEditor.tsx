      ...
      {courses.map((c) => (
        <details key={c.id} className="bg-white rounded-2xl border border-line">
          <summary className="cursor-pointer px-6 py-4 flex flex-wrap items-center gap-3">
            <span className="font-bold text-green">{c.name}</span>
            <span className="text-[10px] font-extrabold uppercase bg-mist rounded-full px-2.5 py-1">{c.type}</span>
          ...
          <div className="px-6 pb-6 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-green">Levels & Books</h3>
                <button
    ...
    <form action={action} className="bg-mist rounded-xl p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <label className="flex items-center gap-2 text-xs font-bold text-ink">
          <input type="checkbox" name="active" defaultChecked={course.active} className="w-4 h-4 accent-green" />
          Active
        </label>
        <button disabled={pending} className="ml-auto bg-green hover:bg-green-deep disabled:opacity-60 text-white text-xs font-bold rounded-lg px-4 py-2 transition-colors">
          {pending ? "…" : "Save course"}
        </button>
        {state?.ok && <span className="text-xs font-bold text-green">✓</span>}
    ...
    <form action={action} className="border border-line rounded-xl p-4 space-y-2.5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3 text-[11px] font-bold text-ink-soft">
          <label>
            Student Book {level.studentBookUrl && <span className="text-green">(uploaded ✓)</span>}
            <input type="file" name="studentBook" accept="application/pdf" className="block mt-1 text-[10px] file:mr-2 file:rounded file:border-0 file:bg-mist file:px-2 file:py-1 file:text-[10px] file:font-bold" />
          </label>
          <label>
            Work Book {level.workBookUrl && <span className="text-green">(uploaded ✓)</span>}
            <input type="file" name="workBook" accept="application/pdf" className="block mt-1 text-[10px] file:mr-2 file:rounded file:border-0 file:bg-mist file:px-2 file:py-1 file:text-[10px] file:font-bold" />
          </label>
        </div>
        <button disabled={pending} className="ml-auto bg-green hover:bg-green-deep disabled:opacity-60 text-white text-[11px] font-bold rounded-lg px-4 py-2 transition-colors">
          {pending ? "…" : "Save level"}
        </button>
        {state?.ok && <span className="text-xs font-bold text-green">✓</span>}
        {state?.error && <span className="text-xs font-semibold text-red-600">{state.error}</span>}
