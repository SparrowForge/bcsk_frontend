...
export default async function AssignmentsPage() {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="font-display text-2xl font-semibold text-green mb-6">Assignment & Homework</h1>
          ...
          return (
            <div key={a.id} className="bg-white rounded-2xl border border-line p-6">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-green-mid">{a.classSession.title}</p>
                  <h2 className="mt-1 font-bold text-ink">{a.title}</h2>
                </div>
                {a.dueDate && (
                  <span className={`text-xs font-bold rounded-full px-3 py-1 ${overdue ? "bg-red-50 text-red-600" : "bg-mist text-green"}`}>
              ...
              {a.fileUrl && (
                <a href={`/api/files/${a.fileUrl}`} className="inline-block mt-3 text-green-mid text-sm font-bold hover:underline">
                  Download task file
                </a>
              )}
              <div className="mt-4 border-t border-line pt-4">
                {sub ? (
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="text-xs font-bold text-white bg-green rounded-full px-3 py-1">Submitted</span>
                    <span className="text-xs text-ink-soft">
                      {formatDate(sub.submittedAt)}
                    </span>
                    {sub.grade ? (
                      <span className="text-xs font-bold text-green bg-green-soft rounded-full px-3 py-1">Grade: {sub.grade}</span>
                    ) : (
                      <span className="text-xs text-ink-soft">Awaiting grade</span>
                    )}
                    {sub.feedback && <p className="w-full text-xs text-ink bg-mist rounded-lg p-3 mt-1">Teacher: {sub.feedback}</p>}
                  </div>
                ) : (
