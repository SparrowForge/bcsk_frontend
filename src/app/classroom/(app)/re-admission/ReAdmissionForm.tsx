...
export function ReAdmissionForm({ amount, classLabel, semester }: { amount: number; classLabel: string; semester: string }) {
  ...
  return (
    <div className="bg-white border border-line rounded-2xl p-6">
      <dl className="bg-mist rounded-xl p-4 text-sm space-y-1.5 mb-5">
      ...
      <form action={action} className="space-y-4">
        ...
        <button
          disabled={pending}
          className="w-full bg-crimson hover:bg-crimson-deep disabled:opacity-60 text-white font-bold rounded-lg px-6 py-3 text-sm transition-colors"
        >
