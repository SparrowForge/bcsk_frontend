...
export default async function StudentDashboard() {
  ...
  const { profile, enrollments, notifications: reminders } = await classroom.dashboard();
    ...
    <div className="space-y-8">
      ...
      <section className="bg-white rounded-3xl border border-line p-6 sm:p-8 flex flex-wrap items-center gap-6">
        {profile?.photoUrl ? (
          ...
          <div className="w-20 h-20 rounded-full bg-green-mid flex items-center justify-center text-3xl font-display font-semibold text-white">
            {session.name[0]}
          </div>
        )}
        <div>
        ...
        <div className="ml-auto hidden sm:block text-right text-xs text-ink-soft">
          <p className="font-bold text-green">Bangladesh Community School, Korea</p>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
              ...
              return (
                <div key={e.id} className="bg-white rounded-2xl border border-line p-5 flex flex-col">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-bold text-green leading-snug">{cs.title}</h3>
                    {cs.isLive ? (
                      ...
                      <span className="shrink-0 text-[10px] font-bold text-ink-soft bg-mist rounded-full px-2.5 py-1">
                        {cs.dayOfWeek}
                      </span>
                    )}
                  </div>
                  ...
                  {cs.notice && <p className="mt-2 text-xs bg-mist rounded-lg p-2.5 text-ink">{cs.notice}</p>}
                  <div className="mt-auto pt-4">
                    {cs.isLive && cs.zoomLink ? (
                      <a
                        ...
                        className="inline-block bg-crimson hover:bg-crimson-deep text-white text-xs font-bold rounded-lg px-4 py-2 transition-colors"
                      >
                        Join Zoom class
                      </a>
                    ) : (
            ...
            {enrollments.length === 0 && (
              <p className="text-sm text-ink-soft bg-white rounded-2xl border border-line p-6 sm:col-span-2">
                No active enrollments this semester. Use <Link className="text-green-mid font-bold" href="/classroom/re-admission">Re-Admission</Link> to enroll.
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {quick.map((q) => (
              ...
              >
                ...
                <span className="block mt-2 text-xs font-bold text-green leading-snug">{q.label}</span>
              </Link>
            ))}
          </div>
        </section>
